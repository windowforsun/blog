...
excerpt: 'LangGraph 를 사용해 RAG 의 기본적인 형태인 Naive RAG 를 구성하는 방법에 대해 알아보자'
...
이제 구현한 `Relevant Checker RAG` 구조를 실행한다.  
...
# /tmp/ipython-input-34-407285766.py in <cell line: 0>()
# 7 inputs = GraphState(question='비트코인 가격 정리해줘')
# 8
# ----> 9 execute_graph(graph, config, inputs)
# 
# 1 frames
# /usr/local/lib/python3.11/dist-packages/langgraph/pregel/__init__.py in stream(self, input, config, stream_mode, print_mode, output_keys, interrupt_before, interrupt_after, checkpoint_during, debug, subgraphs)
# 2557                     error_code=ErrorCode.GRAPH_RECURSION_LIMIT,
# 2558                 )
# -> 2559                 raise GraphRecursionError(msg)
# 2560             # set final channel values as run output
# 2561             run_manager.on_chain_end(loop.output)
# 
...
---  
## Reference
[Naive RAG](https://wikidocs.net/267809)  
